...
export const PokemonNamesList = (): JSX.Element => {
  ...
  const prevNames = React.useRef<string[]>([]);
  let { data, error } = useSWR<IAllPokemonApiInfo>(urlToFetch, get);

  if (error) return <div>Error!</div>;
  // if (!data) return <div>Loading...</div>;

  //To prevent page blinks
  let pokemonNames = data?.results.map((info) => info.name) || prevNames.current;
  prevNames.current = pokemonNames;
